...
def get_cached_login_css(theme: str) -> str:
    css_vars = """
        ...
        --nav-bg: rgba(3, 7, 18, 0.75); --card-bg: rgba(255,255,255,0.015); --card-hover: rgba(255,255,255,0.03);
    ...
    .block-container {{ padding-top: 140px !important; padding-bottom: 0 !important; max-width: 1400px !important; }}
    .floating-nav-container {{ position: fixed; top: 1.5rem; left: 50%; transform: translateX(-50%); width: 92%; max-width: 1250px; height: 68px; background: var(--nav-bg); backdrop-filter: blur(24px); -webkit-backdrop-filter: blur(24px); border: 1px solid var(--border-heavy); border-radius: 100px; display: flex; justify-content: space-between; align-items: center; padding: 0 1.5rem 0 2rem; z-index: 999999; box-shadow: 0 20px 40px -15px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.05); transition: all 0.4s ease; }}
    .nav-brand-group {{ display: flex; align-items: center; gap: 0.8rem; text-decoration: none; }}
    .nav-logo {{ font-size: 1.15rem; font-weight: 800; color: var(--text-main); display: flex; align-items: center; gap: 0.4rem; letter-spacing: -0.5px; transition: color 0.2s; cursor: pointer; user-select: none; }}
    ...
    .theme-toggle-btn {{ cursor: pointer; font-size: 1.25rem; user-select: none; transition: transform 0.2s; }}
    .theme-toggle-btn:hover {{ transform: scale(1.1) rotate(15deg); }}
    .nav-cta {{ display: inline-flex; align-items: center; background: var(--card-bg); border: 1px solid var(--border-heavy); color: var(--text-main); text-decoration: none; padding: 0.5rem 1.4rem; border-radius: 100px; font-size: 0.85rem; font-weight: 700; transition: all 0.3s ease; white-space: nowrap; }}
    .nav-cta:hover {{ background: var(--text-main); color: var(--bg-main); box-shadow: 0 5px 20px rgba(16,185,129,0.2); transform: translateY(-1px); }}
    ...
    [data-testid="stFormSubmitButton"] > button {{ border-radius: 8px !important; background: var(--btn-bg) !important; border: none !important; padding: 0.8rem !important; margin-top: 1.5rem !important; transition: all 0.3s cubic-bezier(0.16, 1, 0.3, 1) !important; box-shadow: 0 4px 6px rgba(0,0,0,0.1) !important; }}
    ...
    [data-testid="stFormSubmitButton"] > button p {{ color: var(--btn-text) !important; font-weight: 800 !important; font-size: 0.95rem !important; }}
    .lp-section {{ padding: 8rem 10%; border-top: 1px solid var(--border-light); scroll-margin-top: 80px; margin-top: 4rem; }}
...
def login_screen():
    ...
    with col_auth:
        ...
        if submit:
            # Note for Scale: In a high-traffic production build, wrap get_db() with @st.cache_resource 
            # to implement Database Connection Pooling.
            conn = get_db()
            cur  = conn.cursor()
