...
def get_cached_login_css(theme: str) -> str:
    ...
    .stApp {{
        background-image:
            radial-gradient(circle at 15% 50%, rgba(16,185,129,0.04), transparent 45%),
    ...
    .lt-brand:hover .lt-logo {{ color: #10b981; }}
    .lt-divider {{ width: 1px; height: 22px; background: var(--border-heavy); flex-shrink: 0; }}
    .lt-subtitle {{
        font-size: 0.68rem;
        font-weight: 700;
        color: var(--text-sub);
    ...
    .lt-link:hover {{ color: #10b981 !important; text-decoration: none !important; }}
    .lt-link:hover::after {{ width: 100%; }}
    ...
    .lt-toggle:hover {{ transform: rotate(15deg) scale(1.1); border-color: #10b981; }}
    .lt-cta {{
        display: inline-flex;
        align-items: center;
        background: var(--card-bg);
    ...
    .brand-desc {{
        font-size: 1.1rem; color: var(--text-sub); line-height: 1.7;
        max-width: 95%; font-weight: 400; margin-bottom: 3.5rem;
    }}
    ...
    .corridor-map {{
        ...
        margin-top: 2.5rem; width: 85%; opacity: 0.8;
    }}
    .corridor-line {{
        position: absolute; width: 100%; height: 2px;
        background: var(--border-heavy); top: 50%; transform: translateY(-50%);
    }}
    .corridor-flow {{
        position: absolute; width: 30%; height: 2px;
        background: linear-gradient(90deg, transparent, #10b981, transparent);
        top: 50%; transform: translateY(-50%);
        animation: freightFlow 3s infinite linear;
    }}
    .corridor-node {{
        position: absolute; display: flex; flex-direction: column;
        align-items: center; gap: 6px;
        top: 50%; transform: translate(-50%, -12px);
    }}
    .node-dot {{
        width: 10px; height: 10px; background: var(--bg-main);
        border: 2px solid #3b82f6; border-radius: 50%; z-index: 2; transition: all 0.3s;
    }}
    .corridor-node:hover .node-dot {{
        background: #10b981; border-color: #10b981;
        box-shadow: 0 0 15px rgba(16,185,129,0.5); transform: scale(1.3);
    }}
    .node-label {{
        font-size: 0.65rem; font-weight: 700;
        color: var(--text-muted); letter-spacing: 1px; transition: color 0.3s;
    }}
    ...
    .auth-sub {{
        ...
        margin-bottom: 2rem; font-weight: 500; line-height: 1.5;
    }}
    .stTextInput label {{
        color: var(--text-sub) !important; font-size: 0.75rem !important;
        font-weight: 700 !important; text-transform: uppercase;
        letter-spacing: 1px; margin-bottom: 0.5rem !important;
    }}
    .stTextInput input {{
        ...
        color: var(--text-main) !important;
        padding: 0.8rem 1rem !important; font-size: 0.95rem !important; transition: all 0.2s;
    }}
    ...
    [data-testid="stFormSubmitButton"] > button {{
        border-radius: 8px !important; background: var(--btn-bg) !important;
    ...
    /* ── LANDING SECTIONS ───────────────────────────────────────────────── */
    .lp-section {{
        padding: 8rem 10%; border-top: 1px solid var(--border-light);
        scroll-margin-top: 80px; margin-top: 4rem;
    }}
    .lp-header {{
        font-size: 2.2rem; font-weight: 700; letter-spacing: -1px;
        margin-bottom: 1rem; color: var(--text-main); text-align: center;
    }}
    .lp-sub {{
        font-size: 1.05rem; color: var(--text-sub); text-align: center;
        max-width: 700px; margin: 0 auto 4rem auto; line-height: 1.6; font-weight: 400;
    }}
    ...
    .card-icon {{
        ...
        background: var(--border-mid); display: flex; align-items: center;
    ...
    .lp-tag {{
        ...
        background: var(--card-bg); border: 1px solid var(--border-heavy);
    ...
    .lp-tag:hover {{
        ...
        box-shadow: 0 5px 15px -5px rgba(16,185,129,0.15);
    }}
    .lp-footer {{
        background: var(--footer-bg); padding: 4rem 10%;
...
def get_cached_hero_html() -> str:
    ...
    <div class='brand-desc'>The centralized operating system for Pakistan's freight network.
    Command your fleet, track shipments, and coordinate dispatch operations
    with unprecedented clarity.</div>
...
@st.cache_data(show_spinner=False)
def get_cached_marketing_html() -> str:
    return """
    <section id='network' class='lp-section'>
    <div class='lp-header'>Centralized Operations</div>
    <div class='lp-sub'>LogiTrack PK replaces fragmented workflows with a single source of truth.
    Command your fleet, coordinate dispatch operations, and track deliveries with unparalleled visibility.</div>
    <div class='grid-3'>
    ...
    <div class='grid-2'>
    <div>
        <div class='lp-header' style='text-align:left;'>Architected for
            <span style='color:#10b981;font-weight:800;text-shadow:0 0 20px rgba(16,185,129,0.2);'>Pakistan's</span> Ecosystem
        </div>
        <div class='lp-sub' style='text-align:left;margin-bottom:2rem;'>
            Designed for the operational realities of the domestic supply chain. Serving the critical transport
            corridors connecting <b>Karachi, Lahore, Islamabad, Faisalabad, Multan, Peshawar, and Quetta.</b>
        </div>
        <div class='tag-wrap'>
            <span class='lp-tag'>Freight Forwarders</span><span class='lp-tag'>3PL Providers</span>
            <span class='lp-tag'>Fleet Operators</span><span class='lp-tag'>Warehouse Managers</span>
            <span class='lp-tag'>Dispatch Coordinators</span><span class='lp-tag'>Intercity Networks</span>
        </div>
    </div>
    <div style='background:rgba(16,185,129,0.03);border:1px solid rgba(16,185,129,0.1);border-radius:24px;
                padding:3rem;display:flex;flex-direction:column;align-items:center;justify-content:center;'>
        <div style='width:100px;height:100px;border-radius:50%;border:1px solid rgba(16,185,129,0.3);
                    display:flex;align-items:center;justify-content:center;position:relative;'>
            <div style='width:60px;height:60px;border-radius:50%;background:rgba(16,185,129,0.1);
                        display:flex;align-items:center;justify-content:center;'>
            ...
            <div style='position:absolute;width:100%;height:100%;border-radius:50%;
                        border:2px solid #10b981;border-top-color:transparent;animation:spin 4s linear infinite;'></div>
        </div>
    ...
    <div class='grid-3'>
        <div class='lp-card' style='text-align:center;padding:3rem 2rem;'>
            ...
            <div class='card-desc' style='font-size:0.85rem;'>Backend Architecture</div>
        </div>
        <div class='lp-card' style='text-align:center;padding:3rem 2rem;'>
            <div style='width:56px;height:56px;border-radius:50%;background:var(--border-mid);margin:0 auto 1.2rem auto;
                        display:flex;align-items:center;justify-content:center;font-size:1.1rem;
    ...
    <div class='lp-sub'>For issue reporting, technical assistance, or feature requests, contact our engineering team.</div>
    <a href='mailto:u2024585@giki.edu.pk'
       style='display:inline-block;background:var(--btn-bg);color:var(--btn-text) !important;
              padding:0.9rem 2.5rem;border-radius:8px;font-weight:700;text-decoration:none !important;
              font-size:0.95rem;transition:all 0.3s;box-shadow:0 10px 25px var(--border-heavy);'>
        u2024585@giki.edu.pk
    </a>
    <div style='margin-top:1.5rem;color:var(--text-muted);font-size:0.85rem;font-weight:600;'>
        Expected Response SLA: Under 24 Hours
    </div>
    </section>
...
def login_screen():
    ...
    <nav class="lt-nav" id="lt-nav">
        ...
        <div class="lt-links">
            <a href="#network"      class="lt-link">Operations</a>
            <a href="#ecosystem"    class="lt-link">Freight Network</a>
            <a href="#capabilities" class="lt-link">Platform Modules</a>
            <a href="#team"         class="lt-link">Engineering</a>
        </div>
        <div class="lt-controls">
            <div class="lt-toggle" id="lt-theme-btn" title="Toggle theme">{toggle_icon}</div>
            <a href="#auth-portal" class="lt-cta">System Access</a>
        </div>
    </nav>
...
if not st.session_state.user:
    ...
    st.markdown("<img src='x' style='display:none;' onerror=\"document.body.classList.remove('light-theme');\">",
                unsafe_allow_html=True)
    ...
    with hdr_left:
        ...
        <div style='display:flex;align-items:center;gap:0.75rem;'>
            ...
            <span style='color:#94a3b8;font-size:0.62rem;font-weight:700;letter-spacing:2.5px;text-transform:uppercase;
                         padding:2px 10px;border:1px solid rgba(255,255,255,0.1);border-radius:20px;'>Enterprise</span>
        </div>
        """, unsafe_allow_html=True)
    with hdr_right:
    ...
    with nav_col:
        ...
        accent, bg_c, border_c = role_colors.get(
            user_role, ("#64748b", "rgba(255,255,255,0.05)", "rgba(255,255,255,0.1)")
        )
